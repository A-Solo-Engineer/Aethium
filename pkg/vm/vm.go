...
// ExceptHandler records where to jump when an exception is raised.
...
// WithBlock holds context-manager cleanup info for `with` statements.
type WithBlock struct {
	exitFn Value
	sp     int
}

// VM is the Aethium bytecode virtual machine.
type VM struct {
	constants []Value
	names     []string
...
// Bytecode is the compiled program representation (mirrors compiler.Bytecode).
...
// Config allows embedding hosts to customise the VM.
...
// Globals returns a thread-safe snapshot of global variables.
...
// ImportModule imports a module by name using the registered loader or fallback lookup.
...
// WaitGoroutines blocks until all background goroutines spawned by this VM (and its children) finish.
...
// DrainGoroutineErrors returns all recorded goroutine errors so far and clears the buffer.
...
// WaitAndDrainGoroutineErrors blocks until all spawned goroutines complete and returns all collected errors.
...
// runtimeError builds an ExceptionObject with a traceback.
...
// handleException tries to dispatch exc to the innermost handler.
// Returns nil if handled (execution continues), or the exc as error if unhandled.
...
func (vm *VM) Run() (retErr error) {
	...
	for {
		insCount++
		// Periodically check context cancellation (every 1024 instructions).
		if insCount&1023 == 0 {
			select {
			case <-vm.ctx.Done():
		...
		default:
			// Unknown opcodes are silently skipped to allow forward compatibility.
		}
	}
}

// isIdentical uses pointer equality for reference types, value equality for primitives.
...
func (vm *VM) executeCall(numArgs, numKwargs int) error {
	...
	case *ClosureObject:
		// Pop args (they are already on the stack above calleeIdx).
		args := make([]Value, numArgs)
		for i := numArgs - 1; i >= 0; i-- {
			args[i] = vm.pop()
		}
	...
	case *ClassObject:
		...
		if initMethod, ok := fn.LookupMethod("__init__"); ok {
			...
			if closure, ok := initMethod.(*ClosureObject); ok {
				// Put self at calleeIdx, then args.
				...
				// Mark frame as __init__ so return pushes self.
...
// callClosure sets up a new frame for a closure call with proper default/vararg/kwarg handling.
func (vm *VM) callClosure(fn *ClosureObject, calleeIdx int, args []Value, kwargs *DictObject) error {
	...
	if kwArg != "" {
		...
		// Apply named kwargs to matching params.
		for _, pair := range kwargs.Pairs {
...
func (vm *VM) evalFloorDiv(l, r Value) (Value, error) {
	...
	if okL && okR {
		...
		result := li / ri
		// Python semantics: floor toward negative infinity.
...
func (vm *VM) evalMod(l, r Value) (Value, error) {
	if l.Type() == TypeInt && r.Type() == TypeInt {
		...
		// Python: result same sign as divisor.
