...
// GetBuiltins builds the ordered builtin table. vm is passed so builtins that
// need context-aware behaviour (e.g. super(), print to stdout) can close over it.
func GetBuiltins(machine *VM) []*BuiltinFunction {
	// super() needs access to the current frame at call time, so we wrap it.
...
func (vm *VM) builtinExit(args ...Value) (Value, error) {
	...
	// Return ExitError so embedding hosts can catch it; the engine
	// decides whether to call os.Exit based on Config.NoExit.
	return None, &ExitError{Code: code}
}

// builtinSuper returns a SuperObject that proxies method lookup past the
// current class in the MRO. Called without arguments (Python-3 style)
// it inspects the current frame to find `self` and the enclosing class.
func (vm *VM) builtinSuper(args ...Value) (Value, error) {
	...
	// Zero-argument super(): inspect call stack for self and __class__.
	if vm.frameIndex >= 2 {
...
// builtinFilter returns a lazy iterator that keeps truthy-tested elements.
...
func builtinSorted(args ...Value) (Value, error) {
	...
	// reverse= kwarg check (basic: if second arg is truthy bool).
	reverse := false
	if len(args) > 1 {
...
// builtinIsInstance supports MRO-aware class checking and primitive type names.
func builtinIsInstance(args ...Value) (Value, error) {
	...
	// Handle tuple of types as second argument.
	...
	// Primitive type names via string.
...
// ExtractIterable materialises any Aethium iterable into a Value slice.
func ExtractIterable(v Value) []Value {
	...
	case *RangeIterator:
		// Materialise remaining items from the lazy iterator.
		var items []Value
		for {
			v, ok := obj.Next()
			if !ok {
...
// LessThan implements < with numeric and string comparison.
...
func callStringMethod(obj StringValue, name string, args []Value) (Value, error) {
	...
	case "encode":
		return StringValue(s), nil // simplified: just return as-is
	}
