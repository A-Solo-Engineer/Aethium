...
// ParseProgram parses top-level statements
func (p *Parser) ParseProgram() *ast.Program {
	...
	for !p.curTokenIs(token.EOF) {
		...
		p.skipNewlines()
		// Guard against infinite loop if parser didn't advance
...
func (p *Parser) parseBlock() *ast.BlockStatement {
	...
	// Next should be NEWLINE followed by INDENT, or a single-line statement
	p.nextToken()
...
func (p *Parser) parseFromImportStatement() *ast.FromImportStatement {
	...
	p.nextToken() // start of imported names
...
func (p *Parser) parseGoSpawnStatement() *ast.GoSpawnStatement {
	...
	p.nextToken() // consume 'go' or 'spawn'
	expr := p.parseExpression(LOWEST)
...
func (p *Parser) parseExpressionOrAssignStatement() ast.Statement {
	...
	// Check for tuple/multiple targets on left side of assignment: a, b = 1, 2
	...
	if len(targets) > 0 {
		// It's a tuple expression statement
...
func (p *Parser) parseFStringLiteral() ast.Expression {
	...
	for i < len(raw) {
		if raw[i] == '{' {
			...
			// Parse exprStr using mini parser
...
func (p *Parser) parsePowerExpression(left ast.Expression) ast.Expression {
	...
	// Right-associative: parse with POWER - 1
	p.nextToken()
...
func (p *Parser) parseListLiteralOrComp() ast.Expression {
	...
	// Check if this is a list comprehension: [elem for var in iter if cond]
	if p.peekTokenIs(token.FOR) {
		...
		p.nextToken() // move to start of iterable
		iter := p.parseExpression(IF_ELSE)

		var cond ast.Expression
		if p.peekTokenIs(token.IF) {
			p.nextToken() // curToken is 'if'
			p.nextToken() // move to start of condition
			cond = p.parseExpression(LOWEST)
		}
...
func (p *Parser) parseDictOrSetOrComp() ast.Expression {
	...
	if p.curTokenIs(token.RBRACE) {
		// Empty {} in Python is an empty dict
		return &ast.DictLiteral{PosInfo: pos, Entries: []ast.DictEntry{}}
	}

	firstExpr := p.parseExpression(LOWEST)

	// If colon follows, this is a Dict or DictComp
	if p.peekTokenIs(token.COLON) {
		p.nextToken() // consume ':'
		p.nextToken()
		valExpr := p.parseExpression(LOWEST)

		// Check for DictComp: {k: v for var in iter if cond}
		if p.peekTokenIs(token.FOR) {
			p.nextToken() // consume 'for'
			p.nextToken()
			target := p.parseExpression(COMPARISON)
	...
	// Otherwise, it's a Set or SetComp: {x for x in iter}
	if p.peekTokenIs(token.FOR) {
		p.nextToken() // consume 'for'
		p.nextToken()
		target := p.parseExpression(COMPARISON)
