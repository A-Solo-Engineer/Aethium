...
// Banner returns the branded welcome text with centralized version.
func Banner() string {
    return fmt.Sprintf("Aethium %s (Fast Python-like runtime in Go)\nType \"exit()\", \"quit()\", or Ctrl+D to exit.\n", engine.Version)
}

// ANSI colors for syntax highlighting in REPL
const (
    ColorReset  = "\033[0m"
    ColorCyan   = "\033[36m"
    ColorGreen  = "\033[32m"
...
func Start(in io.Reader, out io.Writer) {
    ...
    for {
        ...
        if !inBlock {
            ...
            // Execute single complete line
            executeLine(eng, line, out)
        } else {
            if line == "" {
                // Empty line in multiline block ends block input
                inBlock = false
                ...
                multilineBuffer = append(multilineBuffer, line)
                // If closing brackets balance and line doesn't end with colon, test compilation
                fullSource := strings.Join(multilineBuffer, "\n")
...
// isIncomplete checks if the source has unclosed brackets, quotes, or ends with a colon.
func isIncomplete(src string) bool {
    ...
    // Try parsing; if it fails with unexpected EOF/indentation, it's incomplete
